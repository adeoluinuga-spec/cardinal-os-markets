...
export async function POST(request: Request) {
  ...
  const [customers, products, orders] = await Promise.all([
    ...
    supabase.from("orders").select("order_number, status, channel, total, notes").eq("tenant_id", t.id).or(`order_number.ilike.${like},status.ilike.${like},channel.ilike.${like},notes.ilike.${like}`).limit(8),
  ]);
  const results: Result[] = [
    ...
    ...(orders.data ?? []).map((order) => ({ title: order.order_number, source: "Order", excerpt: `${order.status} - ${order.channel} - Total NGN ${Number(order.total ?? 0).toLocaleString("en-NG")}` })),
  ];
